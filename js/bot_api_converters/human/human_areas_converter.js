...
export class HumanAreasConverter extends HumanConverter {
    ...
    */
    ...
    toRaw(areas) {
        ...
        for (var i = 0; i < areas.areas.length; i++) {
            var w = Math.abs(areas.areas[i].startPosition.x - areas.areas[i].endPosition.x) +1;
            var h = Math.abs(areas.areas[i].startPosition.y - areas.areas[i].endPosition.y) +1 ;
            output += `Area ${i}: W: ${w} H: ${h} A: ${w*h} \nArea ${i}: ${areas.areas[i].startPosition.x},${areas.areas[i].startPosition.y},${areas.areas[i].endPosition.x},${areas.areas[i].endPosition.y}\n`;
        }
        return output;
    }
